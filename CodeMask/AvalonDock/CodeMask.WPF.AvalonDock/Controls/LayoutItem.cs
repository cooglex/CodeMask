...
namespace CodeMask.WPF.AvalonDock.Controls
{
    public abstract class LayoutItem : FrameworkElement
    {
        ...
        static LayoutItem()
        {
            ToolTipProperty.OverrideMetadata(typeof (LayoutItem),
                new FrameworkPropertyMetadata(null, (s, e) => OnToolTipChanged(s, e)));
            VisibilityProperty.OverrideMetadata(typeof (LayoutItem),
                new FrameworkPropertyMetadata(Visibility.Visible, (s, e) => OnVisibilityChanged(s, e)));
        }
        ...
        {
            Trace.WriteLine(string.Format("Detach({0})", LayoutElement.Title));
            LayoutElement.IsSelectedChanged -= LayoutElement_IsSelectedChanged;
            LayoutElement.IsActiveChanged -= LayoutElement_IsActiveChanged;
            LayoutElement = null;
            Model = null;
        ...
        protected virtual void InitDefaultCommands()
        {
            ...
            _defaultDockAsDocumentCommand = new RelayCommand(p => ExecuteDockAsDocumentCommand(p),
                p => CanExecuteDockAsDocumentCommand(p));
            _defaultCloseAllButThisCommand = new RelayCommand(p => ExecuteCloseAllButThisCommand(p),
                p => CanExecuteCloseAllButThisCommand(p));
            _defaultActivateCommand = new RelayCommand(p => ExecuteActivateCommand(p), p => CanExecuteActivateCommand(p));
            _defaultNewVerticalTabGroupCommand = new RelayCommand(p => ExecuteNewVerticalTabGroupCommand(p),
                p => CanExecuteNewVerticalTabGroupCommand(p));
            _defaultNewHorizontalTabGroupCommand = new RelayCommand(p => ExecuteNewHorizontalTabGroupCommand(p),
                p => CanExecuteNewHorizontalTabGroupCommand(p));
            _defaultMoveToNextTabGroupCommand = new RelayCommand(p => ExecuteMoveToNextTabGroupCommand(p),
                p => CanExecuteMoveToNextTabGroupCommand(p));
            _defaultMoveToPreviousTabGroupCommand = new RelayCommand(p => ExecuteMoveToPreviousTabGroupCommand(p),
                p => CanExecuteMoveToPreviousTabGroupCommand(p));
        }
...
#if DEBUG
            if (LayoutElement != null)
                Trace.WriteLine(string.Format("CanExecuteCloseCommand({0}) = {1}", LayoutElement.Title,
                    LayoutElement.CanClose));
#endif
        ...
        public static readonly DependencyProperty MoveToNextTabGroupCommandProperty =
            DependencyProperty.Register("MoveToNextTabGroupCommand", typeof (ICommand), typeof (LayoutItem),
                new FrameworkPropertyMetadata(null,
                    OnMoveToNextTabGroupCommandChanged));
