...
namespace Standard
{
    ...
    internal enum GPS
    {
        ...
        // A read/write store that only holds properties for the lifetime of the IShellItem object
    ...
    internal enum SHCONTF
    {
        ...
        // hint that client is checking if (what) child items the folder contains - not all details (e.g. short file name) are needed
        ...
        // mark child folders to indicate that they should provide a "navigation" enumeration by default
        ...
        ENABLE_ASYNC = 0x8000
        // inform enumerator that client is listening for change notifications so enumerator does not need to be complete, items can be reported via change notifications
    }
    ...
    ///     in the bind context
    ///     Here are the string versions of GPS_ flags, passed to IShellFolder::BindToObject() via
    ...
    internal struct PKEY
    {
        ...
        public static readonly PKEY AppUserModel_IsDestListSeparator =
            new PKEY(new Guid("9F4C2855-9F79-4B39-A8D0-E1D42DE1D5F3"), 6);

        /// <summary>PKEY_AppUserModel_RelaunchCommand</summary>
        public static readonly PKEY AppUserModel_RelaunchCommand =
            new PKEY(new Guid("9F4C2855-9F79-4B39-A8D0-E1D42DE1D5F3"), 2);
    ...
    internal interface IEnumObjects
    {
        ...
        void Next(uint celt, [In] ref Guid riid,
            [Out,
             MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.IUnknown, IidParameterIndex = 1,
                 SizeParamIndex = 0)] object[] rgelt, [Out] out uint pceltFetched);
